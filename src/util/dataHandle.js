...
let Auth = {
    delToken() {
        localStorage.removeItem(KEY_64);
    },
    setToken(token) {
        localStorage.setItem(KEY_64, token);
    },
    getToken() {
        return localStorage.getItem(KEY_64);
    },
    // 设置过期时间
    setExpTime() {
        let curTime = new Date().getTime();
        localStorage.setItem(expTime, curTime);
    },
    isExpires(expTime) {
        let oldTime = localStorage.getItem(expTime);
        let curTime = new Date().getTime()
        if(curTime - oldTime > expTime){
            this.delToken()
        }
    },
}
